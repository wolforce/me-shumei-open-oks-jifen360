...
public class Signin extends CommonData {
    ...
    public String[] start(Context ctx, boolean isAutoSign, String cfg, String user, String pwd) {
        ...
        try{
            ...
            String scorelistUrl = "http://jifen.360.cn/index/scorelist";//个人中心页面URL，从此页面可以提取出访问签到URL用的token，且只需16KB流量，比从120KB的首页中提取的好
            ...
            if(isLoginSucceed)
            {
                ...
                else
                {
                    ...
                    res = Jsoup.connect(scorelistUrl).cookies(cookies).userAgent(UA_ANDROID).timeout(TIME_OUT).ignoreContentType(true).method(Method.GET).execute();
                    cookies.putAll(res.cookies());
     ...
     * http://i.360.cn/login/
     * 登录步骤
     * 1.手动构造两个Cookies：i360loginName=wolforce%2540shumei.me，trad=0
